...
import { getUserByMatricula } from '../services/api'; // Import the API function
...
function Unumber() {
  ...
  const buttonpressed = async () => {
    if (inputRef.current) {
      ...
      try {
        const user = await getUserByMatricula(matricula); // Fetch user data
        console.log(user)
        if (user.length > 0) {
          setUser(user[0]); // Set the fetched user data in context
          setError('');
